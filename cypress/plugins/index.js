...
module.exports = (on, config) => {
  ...
  on("task", {
    lighthouse: lighthouse((lighthouseReport) => {
      ...
      fs.writeFileSync(
        `${dirPath}/GLH-(${auditReportName}).json`,
      ...
      fs.writeFileSync(
        `${dirPath}/GLH-(${categoriesReportName}).json`,
  ...
  //       fs.mkdirSync(dirPath)
  //     }
  //     const name = (lighthouseReport.lhr.requestedUrl).replace(/[-[\]{}()*+!<=:?.\/\\^$|#\s,]/g, function (x) { return '' }) + " - " + (lighthouseReport.lhr.fetchTime).split('T')[0] + "lighthouse"
